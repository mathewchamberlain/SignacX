geneset_preprocessing <- function()
{
  ...
  data_dir = "/site/ne/data/bh-results/C/CHAMBERLAIN.Mat/pipelines/Signac/data-raw/"
  ...
  # remove RP genes
  logik = grepl("^RP", rownames(LM22)); sum(logik)
  LM22 = LM22[!logik,]

  ## load expression matrix for epithelial, epithelial urinary, secretory cell types
  data.dir = "/site/ne/data/bh-results/C/CHAMBERLAIN.Mat/spring/SLE_CELLSEQ_v2/"
  spring.dir = "/site/ne/data/bh-results/C/CHAMBERLAIN.Mat/spring/SLE_CELLSEQ_v2/All_SLE_CELLSEQ/"
  ...
  EpUr <- read.delim(paste(data_dir,"selected_cells_EpithelialUrinary.txt",sep=""), sep = "," , stringsAsFactors=FALSE, header = F)$V1 + 1# cell idxs
  SecC <- read.delim(paste(data_dir,"selected_cells_Secretory.txt",sep=""), sep = ",", stringsAsFactors=FALSE, header = F)$V1 + 1# cell idxs

  EpUr = Matrix::rowMeans(E[,EpUr])
  EpUr = data.frame(EpUr)
  rownames(EpUr) <- rownames(E)

  SecC = Matrix::rowMeans(E[,SecC])
  SecC = data.frame(SecC)
  rownames(SecC) <- rownames(E)

  # take row mean to be the average epithelial signature
  logik = json_data$CellType$label_list == "Epithelial"; sum(logik)
  Epis = Matrix::rowMeans(E[,logik])
  Epis = data.frame(Epis);
  rownames(Epis) <- rownames(E)

  ## load expression matrix for Fibroblast cell types
  data.dir = "/site/ne/data/bh-results/C/CHAMBERLAIN.Mat/spring/RA_CELLSEQ_v1/"
  spring.dir = "/site/ne/data/bh-results/C/CHAMBERLAIN.Mat/spring/RA_CELLSEQ_v1/All_RA_CELLSEQ/"
  E <- CID.LoadData(data.dir = data.dir)
  ...
  # take row sum to be the average epithelial signature
  logik = json_data$CellType$label_list == "Fibroblast"; sum(logik)
  Fibs = data.frame(Matrix::rowMeans(E[,logik]))
  rownames(Fibs) = rownames(E);

  LM22 = cbind(LM22, Epis$Epis[match(rownames(LM22), rownames(Epis))])
  LM22 = cbind(LM22, Fibs$Matrix..rowMeans.E...logik..[match(rownames(LM22), rownames(Fibs))])
  LM22 = cbind(LM22, EpUr$EpUr[match(rownames(LM22), rownames(EpUr))])
  LM22 = cbind(LM22, SecC$SecC[match(rownames(LM22), rownames(SecC))])

  LM22 = na.omit(LM22)

  # get genesets from sc data
  pDC <- read.csv(paste(data_dir,"selected_cells_pDC.txt",sep=""), stringsAsFactors=FALSE, header = F)$V1 + 1 # cell idxs
  Erythro <- read.csv(paste(data_dir,"selected_cells_Erythro.txt",sep=""), stringsAsFactors=FALSE, header = F)$V1 + 1 # cell idxs

  ## load expression matrix for T1D
  data.dir = "/site/ne/data/bh-results/C/CHAMBERLAIN.Mat/spring/T1D_all_v2/"
  E <- CID.LoadData(data.dir = data.dir)

  pDCs = Matrix::rowMeans(E[,pDC])
  pDCs <- as.matrix(pDCs)
  rownames(pDCs) <- rownames(E)

  Erythro = Matrix::rowMeans(E[,Erythro])
  Erythro = as.matrix(Erythro)
  rownames(Erythro) <- rownames(E)

  LM22 = cbind(LM22, pDCs[match(rownames(LM22), rownames(pDCs))])
  LM22 = cbind(LM22, Erythro[match(rownames(LM22), rownames(Erythro))])

  LM22 = na.omit(LM22)

  ## load expression matrix for PBMC fresh frozen
  CytoT  <-  read.csv(paste(data_dir,"selected_cells_CytotoxicTcellsFreshFrozen.txt",sep=""), stringsAsFactors=FALSE, header = F)$V1 + 1 # cell idxs
  NKcells <- read.csv(paste(data_dir,"selected_cells_NKcellsFreshFrozen.txt"        ,sep=""), stringsAsFactors=FALSE, header = F)$V1 + 1 # cell idxs
  Platelets <- read.csv(paste(data_dir,"selected_cells_PlateletsFreshFrozen.txt"    ,sep=""), stringsAsFactors=FALSE, header = F)$V1 + 1 # cell idxs
  MCP <-read.csv(paste(data_dir,"selected_cells_MastCellProgenitors.txt"    ,sep=""), stringsAsFactors=FALSE, header = F)$V1 + 1 # cell idxs
  
  data.dir = "/site/ne/data/bh-results/C/CHAMBERLAIN.Mat/spring/PBMCFresh_Frozen/"
  E <- CID.LoadData(data.dir = data.dir)

  NKcells = Matrix::rowMeans(E[,NKcells])
  NKcells <- as.matrix(NKcells)
  rownames(NKcells) <- rownames(E)

  CytoT = Matrix::rowMeans(E[,CytoT])
  CytoT = as.matrix(CytoT)
  rownames(CytoT) <- rownames(E)

  Platelets = Matrix::rowMeans(E[,Platelets])
  Platelets = as.matrix(Platelets)
  rownames(Platelets) <- rownames(E)

  MCP = Matrix::rowMeans(E[,MCP])
  MCP = as.matrix(MCP)
  rownames(MCP) <- rownames(E)
  
  
  LM22 = cbind(LM22, NKcells[match(rownames(LM22), rownames(NKcells))])
  LM22 = cbind(LM22, CytoT[match(rownames(LM22), rownames(CytoT))])
  LM22 = cbind(LM22, Platelets[match(rownames(LM22), rownames(Platelets))])
  LM22 = cbind(LM22, MCP[match(rownames(LM22), rownames(MCP))])
  LM22 = na.omit(LM22)
  ...
  logik = blueprint_encode$main_types %in% c("CD4+ T-cells", "CD8+ T-cells", "B-cells", "Fibroblasts", "HSC", "NK cells", "Monocytes", "Macrophages") & blueprint_encode$types != "Plasma cells";
